 ...
 * @see IWordDetector
 */
public class InsensitiveWordPatternRule extends InsensitiveSingleLineRule {
	 ...
	 */
	public InsensitiveWordPatternRule(IWordDetector detector, String startSequence, String endSequence, IToken token) {
	 ...
	 */
	public InsensitiveWordPatternRule(IWordDetector detector, String startSequence, String endSequence, IToken token, char escapeCharacter) {
	...
	protected boolean endSequenceDetected(ICharacterScanner scanner) {
		...
		if (fBuffer.length() >= fEndSequence.length) {
			for (int i=fEndSequence.length - 1, j= fBuffer.length() - 1; i >= 0; i--, j--) {
				if (!charEqualIgnoreCase(fEndSequence[i],fBuffer.charAt(j))) {
