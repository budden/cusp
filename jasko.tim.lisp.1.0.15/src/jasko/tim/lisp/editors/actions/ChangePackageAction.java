...
public class ChangePackageAction  extends LispAction  {
	...
	public void run() {
		...
		if (offset>0&&editor!=null) {
			...
			String formattedPack = LispUtil.formatSwankPackageName(pack);
			ReplView view = (ReplView) LispPlugin.findView(ReplView.ID);
			if (formattedPack.equalsIgnoreCase(getSwank().getPackage())) {
				String newPrevPackage = getSwank().getPackage();
				//Should never happen????
				...
				String currentPack = getSwank().getPackage();
				if (view.switchPackageIfExists(pack)) {
					prevPackage = LispUtil.formatSwankPackageName(currentPack);
				}
			}
